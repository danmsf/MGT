...
data = pd.read_sql("select * from [dbo].[" + db0['Tables']['Final'] + "] where BATCH_VAL0=217631 order by timestamp",
                   db)
od = pd.read_excel(fp['OD'])
od.columns
...
for i in range(47):
    ...
    ax = axs[i, 1]
    powerSpectrum, freqenciesFound, time, imageAxis = ax.specgram(X.iloc[:, i], Fs=frequency, NFFT=nfft,
                                                                  noverlap=noverlap)
...
for i in range(10):
    ...
    ax = axs[i, 1]
    powerSpectrum, freqenciesFound, time, imageAxis = ax.specgram(projected[:, i], Fs=frequency, NFFT=nfft,
                                                                  noverlap=noverlap)
